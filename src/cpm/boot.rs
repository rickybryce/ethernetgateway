...
/// The catalogue of disks **measured** here never to reach a sign-on.
///
/// Generated by the `#[ignore]` `record_nonbooting` test, which boots every
/// image in every collection through the product's own path. Shipped in the
/// binary for the same reason [`super::layout::repo_disks`] is: it has to be
/// true of disks the operator has not downloaded yet.
fn nonbooting() -> &'static str {
    include_str!("nonbooting.txt")
}

/// `(bytes, sha256) -> why it does not boot`, parsed once.
///
/// The size is in the key so the common case never hashes: an image whose
/// length matches no entry cannot be one, and that is one `stat` against a
/// handful of integers.
fn nonbooting_table() -> &'static std::collections::HashMap<(u64, &'static str), &'static str> {
    static TABLE: std::sync::OnceLock<
        std::collections::HashMap<(u64, &'static str), &'static str>,
    > = std::sync::OnceLock::new();
    TABLE.get_or_init(|| {
        let mut map = std::collections::HashMap::new();
        for line in nonbooting().lines() {
            // A row is `sha size name why`, and the reason is the rest of the
            // line — spaces and all — so the fields are taken one at a time
            // rather than by `split_whitespace().nth(3)`. Everything else in
            // the file is the explanation above it and is skipped **by shape**,
            // not by a line number: a header that grows must not shift the data.
            let Some((sha, rest)) = line.trim_start().split_once(char::is_whitespace) else {
                continue;
            };
            if sha.len() != 64 || !sha.bytes().all(|b| b.is_ascii_hexdigit()) {
                continue;
            }
            let Some((size, rest)) = rest.trim_start().split_once(char::is_whitespace) else {
                continue;
            };
            let Ok(size) = size.parse::<u64>() else { continue };
            let why = match rest.trim_start().split_once(char::is_whitespace) {
                Some((_name, why)) if !why.trim().is_empty() => why.trim(),
                // A row may name a disk and leave the reason off; saying so
                // beats an empty string on a screen.
                _ => "does not reach a sign-on",
            };
            map.insert((size, sha), why);
        }
        map
    })
}

/// Why this image does not boot, if it is one we have measured and watched fail.
///
/// `None` means "not in the list", which is deliberately **not** the same as
/// "boots" — it is the answer for every disk nobody here has tried, including
/// one the operator built themselves. See `nonbooting.txt` for why the rule
/// runs that way round.
///
/// Cached on the file's identity like [`image_can_boot`], because the desktop
/// panel asks this while redrawing and a 4.9 MB hard disk is not free to hash.
pub fn measured_not_booting(path: &std::path::Path) -> Option<&'static str> {
    ...
    let len = meta.len();
    let table = nonbooting_table();
    // No entry of this length: it cannot be any of them, and nothing is read.
    if !table.keys().any(|(size, _)| *size == len) {
        return None;
    }
    ...
    let key = (path.to_path_buf(), len, meta.modified().ok());
    if let Some(hit) = cache.lock().ok().and_then(|c| c.get(&key).copied()) {
        return hit;
    }
    let verdict = std::fs::read(path)
        .ok()
        .and_then(|bytes| table.get(&(len, super::fetch::sha256(&bytes).as_str())).copied());
...
pub fn image_can_boot(path: &std::path::Path) -> bool {
    ...
    // wrong together.
    //
    // **Measurement outranks the cold-start test**, and only in one direction.
    // `Bootability::offer` asks whether a cold start reaches an entry point,
    // which every IMSAI disk does — and then their loaders drive a controller
    // at port `FDh` that nothing here answers, so they run into silence. No
    // reading of the image can settle that; only booting it can, which is what
    // `nonbooting.txt` records. A disk absent from that list is still offered.
    // **Both reasons to withhold come from one function**, so this cannot
    // disagree with the catalogue or with what a screen says: the disk was
    // booted here and never spoke, or its loader drives a board we do not have.
    // The second exists because the measured list can only ever name disks
    // somebody here has tried — an operator who fetches an IMSAI disk of their
    // own would otherwise be offered it, boot it and watch it go quiet. That
    // detector earns its place only by being narrow: `FDh` is how those loaders
...
pub(crate) mod tests {
    ...
    /// empties, every disk in it comes back onto all three pickers, and
    /// *nothing goes red* — the failure would look exactly like the bug this
    /// list was built to fix, arriving silently. So the count of rows the file
    /// contains and the count the parser produced are held equal: the same
    /// accounting identity the boot sweep uses, and the opposite of the `>= 4`
    /// that once let one parsed row stand in for four.
    #[test]
    fn test_every_row_of_the_nonbooting_list_is_parsed() {
        let looks_like_a_row = |l: &str| {
            l.split_whitespace()
                .next()
                .map(|f| f.len() == 64 && f.bytes().all(|b| b.is_ascii_hexdigit()))
                == Some(true)
        };
        let rows = super::nonbooting().lines().filter(|l| looks_like_a_row(l)).count();
        assert!(rows > 0, "the shipped list has no rows at all — did the recorder run?");
        assert_eq!(
            super::nonbooting_table().len(),
            rows,
            "{rows} rows in nonbooting.txt, {} parsed — a row was dropped or repeated",
            super::nonbooting_table().len()
        );
    }

    /// Every withheld disk states a reason, because the operator sees it.
    ///
    /// A disk removed from the pickers with no cause given is indistinguishable
    /// from one the gateway simply failed to notice.
    #[test]
    fn test_no_disk_is_withheld_without_saying_why() {
        for ((size, sha), why) in super::nonbooting_table() {
            assert!(
                !why.trim().is_empty(),
                "{sha} ({size} bytes) is withheld for no stated reason"
            );
            assert!(*size > 0, "a row with no size cannot match any file: {sha}");
        }
    }

    /// The prose above the table is not data: a growing header must not shift
    /// or fabricate a row.
    #[test]
    fn test_the_files_own_explanation_parses_to_nothing() {
        let header: Vec<&str> = super::nonbooting()
            .lines()
            .take_while(|l| !l.trim_start().starts_with("sha256"))
            .collect();
        assert!(header.len() > 10, "the header really is prose");
        for line in header {
            assert!(
                line.split_whitespace().next().map(|f| f.len() == 64) != Some(true),
                "a line of the explanation parses as a row: {line:?}"
            );
        }
    }

    /// A file nobody has measured is **not** withheld — the safety rule the
    /// whole list rests on, since an operator's own disk is never in it.
    #[test]
    fn test_an_unmeasured_file_is_not_withheld() {
        let dir = std::env::temp_dir().join(format!("nonbooting-{}", std::process::id()));
        ...
        assert_eq!(
            ...
            None,
            "an unknown disk must still be offered"
        );
