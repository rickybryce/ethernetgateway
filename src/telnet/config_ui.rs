    ...
    pub(in crate::telnet) fn config_help_lines(petscii: bool) -> &'static [&'static str] {
        if petscii {
            &[
                ...
                "  L  Cap how many connections one",
                "     address may open, and over",
                "     how many seconds. Refused",
                "     connections never reach the",
                "     login, so this bounds a",
                "     scanner the lockout cannot",
                "     see. 0 turns the cap off.",
                "",
            ...
            &[
                ...
                "  L  Cap how many connections a single address may",
                "     open, and the window in seconds to count them",
                "     over.  A refused connection never reaches the",
                "     login prompt, so this bounds the scanner the",
                "     auth lockout cannot see -- one that opens a",
                "     connection per guess and never submits one.",
                "     0 turns the cap off.",
                "",
