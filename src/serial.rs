...
/// Does this port's erase fold stand to damage a file transfer?
///
/// **Only modem mode can answer yes, and that is the whole point.** A console
/// bridge folds a byte that arrived from a telnet or SSH session -- a human at a
/// terminal, unambiguously, which is why `run_console_bridge` can fold safely --
/// and the Kermit-server wire never folds at all. Modem mode is the one where
/// the folded byte comes off the wire, where a keystroke and a byte of an
/// XMODEM block are indistinguishable to us: `process_online_bytes` sees one
/// stream and cannot tell which is which, so an active fold rewrites payload.
///
/// Used by all three configuration surfaces to mark the setting, so none of them
/// can decide on its own what counts as a risk. It is deliberately **not** a
/// refusal: an operator dialling a remote prompt from a device whose only erase
/// key is DEL has a real use for the fold, and this gateway does not know
/// whether they will ever send a file over the same call.
pub fn erase_fold_risks_transfer(mode: &str, backspace: &str) -> bool {
    mode.trim().eq_ignore_ascii_case("modem") && backspace_target(backspace).is_some()
}

/// The one explanation of that risk, so all three surfaces give the same one.
///
/// Written for a tooltip -- the desktop editor's hover and the web page's
/// `title` -- with [`erase_fold_warning_lines`] carrying the same thing for a
/// terminal, which has no hover to put it in.
pub const ERASE_FOLD_MODEM_WARNING: &str = "This port is in MODEM mode, where an \
active erase fold WILL corrupt a file transfer. Online, the gateway is handed one \
stream of bytes off the wire and cannot tell a keystroke from a byte of an XMODEM \
block, so it rewrites 0x08, 0x7F and 0x14 wherever they fall -- including inside \
the payload of an XMODEM, XMODEM-1K, YMODEM, ZMODEM or Punter upload. (Kermit \
escapes those three, so it survives.) It is the same caveat PETSCII carries, and \
the same remedy: set this back to pass-through before sending a file. Uploads are \
what break -- a download arrives in the other direction, which is never folded.";

/// The marker the serial screens put beside a folding modem port's erase value.
///
/// On the row rather than under it: that screen is 21 of its 22 PETSCII rows and
/// its row-count test models arithmetic rather than the real `send_line` calls,
/// so a conditional row here would go uncaught. Shared with the test that checks
/// it still fits beside the longest choice label, because a marker measured
/// against a copy of itself is not measured at all.
pub const ERASE_FOLD_ROW_MARKER: &str = "  ! folds transfers";

/// [`ERASE_FOLD_MODEM_WARNING`], wrapped for a terminal that has no hover.
///
/// Held as lines rather than wrapped at the call site because the narrowest
/// surface is a 40-column PETSCII C64, where a long line does not wrap -- it
/// loses its end, the same trap `truncate_to_width` exists for. A test measures
/// every line against that width.
pub fn erase_fold_warning_lines() -> &'static [&'static str] {
    &[
        "This port is in MODEM mode, and an",
        "active erase fold WILL corrupt a file",
        "transfer.",
        "",
        "Online, the gateway cannot tell a",
        "keystroke from a byte of an XMODEM",
        "block: it is one stream off the wire.",
        "So 0x08, 0x7F and 0x14 are rewritten",
        "wherever they fall, payload included.",
        "",
        "XMODEM, XMODEM-1K, YMODEM, ZMODEM and",
        "Punter uploads corrupt. Kermit escapes",
        "those three bytes, so it survives, and",
        "a download is never folded.",
        "",
        "Set this back to pass-through before",
        "sending a file -- the same remedy",
        "PETSCII translation carries.",
    ]
}

/// What a port's erase key becomes when its mode changes, if anything.
///
/// **Switching a port to modem mode clears an active fold**, because the fold
/// is safe in the mode being left and hazardous in the one being entered, and a
/// setting that was made for a console bridge should not silently start
/// rewriting a modem's payload. `None` means leave it alone.
///
/// Deliberately the *transition* and not an invariant: "modem mode implies
/// pass-through" would make the setting unreachable in modem mode, and there is
/// a real use for it there (a device whose only erase key is DEL, typing at a
/// remote prompt). An operator who turns it back on is telling us they meant it,
/// and gets [`erase_fold_risks_transfer`] marking the setting instead.
pub fn backspace_after_mode_change(
    old_mode: &str,
    new_mode: &str,
    current: &str,
) -> Option<&'static str> {
    let entering_modem = !old_mode.trim().eq_ignore_ascii_case("modem")
        && new_mode.trim().eq_ignore_ascii_case("modem");
    if entering_modem && backspace_target(current).is_some() {
        Some(BACKSPACE_PASSTHROUGH)
    } else {
        None
    }
}
...
mod backspace_tests {
    ...
    /// **Only modem mode is a risk, and only with the fold on.** The console
    /// bridge folds a byte that came from a telnet session -- a human at a
    /// terminal -- and the Kermit wire never folds at all; modem mode is the one
    /// where a keystroke and a byte of an XMODEM block arrive on the same wire.
    #[test]
    fn test_only_a_folding_modem_port_is_marked() {
        for setting in [BACKSPACE_BS, BACKSPACE_DEL] {
            assert!(erase_fold_risks_transfer("modem", setting), "{setting} in modem mode");
            for safe in ["console", "kermit"] {
                assert!(
                    !erase_fold_risks_transfer(safe, setting),
                    "{setting} in {safe} mode is not a transfer risk -- {safe} does not fold \
                     online data, so marking it would teach the operator to ignore the mark"
                );
            }
        }
        assert!(!erase_fold_risks_transfer("modem", BACKSPACE_PASSTHROUGH));
        // A hand-edited typo folds nothing, so it is not marked -- the same
        // reading `backspace_target` gives it.
        assert!(!erase_fold_risks_transfer("modem", "nonsense"));
        // Case and padding are the operator's, not ours.
        assert!(erase_fold_risks_transfer(" MODEM ", " RuBoUt "));
    }

    /// The clear is on the **transition into** modem mode, not an invariant.
    #[test]
    fn test_entering_modem_mode_clears_the_fold() {
        for from in ["console", "kermit"] {
            assert_eq!(
                backspace_after_mode_change(from, "modem", BACKSPACE_BS),
                Some(BACKSPACE_PASSTHROUGH),
                "switching {from} -> modem leaves the fold on"
            );
        }
        // Already in modem mode: the operator turned it back on deliberately and
        // a save must not undo that, or the setting is unreachable there.
        assert_eq!(backspace_after_mode_change("modem", "modem", BACKSPACE_BS), None);
        // Leaving modem mode changes nothing: the fold is safe where it is going.
        assert_eq!(backspace_after_mode_change("modem", "console", BACKSPACE_BS), None);
        // Nothing to clear.
        assert_eq!(backspace_after_mode_change("console", "modem", BACKSPACE_PASSTHROUGH), None);
    }

    /// **Every surface that can put a port into modem mode applies the rule.**
    ///
    /// Three screens edit this setting and a fourth would be easy to add without
    /// noticing that entering modem mode has to clear the fold, or that a
    /// folding modem port has to be marked. Scanned with comments stripped: a
    /// source-scanning test that reads its own prose passes on the strength of a
    /// doc comment naming the function it is meant to find.
    #[test]
    fn test_all_three_surfaces_apply_the_modem_fold_rule() {
        ...
        for (surface, src) in [
            ("telnet", include_str!("telnet/serial_ui.rs")),
            ("web", include_str!("webserver.rs")),
            ("desktop", include_str!("gui.rs")),
        ] {
            let code = strip_comments(src);
            assert!(
                code.contains("backspace_after_mode_change"),
                "the {surface} screen can set a port to modem mode but never clears \
                 an active fold -- an operator lands in modem mode still folding"
            );
            assert!(
                code.contains("erase_fold_risks_transfer"),
                "the {surface} screen never marks a folding modem port, so that \
                 surface alone gives no sign the fold will corrupt a transfer"
            );
        }
    }

    /// The marker shares the Erase row, so it has to fit beside the value.
    ///
    /// The screen gives the value 58 columns on an 80-column terminal; a longer
    /// choice label added later must not silently push the marker off the end.
    #[test]
    fn test_the_marker_fits_beside_the_longest_choice() {
        const ANSI_VALUE_W: usize = 58;
        let longest = BACKSPACE_CHOICES
            .iter()
            .map(|(_, label)| label.chars().count())
            .max()
            .expect("the list is not empty");
        assert!(
            longest + ERASE_FOLD_ROW_MARKER.chars().count() <= ANSI_VALUE_W,
            "the longest label ({longest} columns) plus the marker ({}) exceeds the \
             {ANSI_VALUE_W} the row gives it -- the marker would be truncated away, \
             leaving red text and no reason",
            ERASE_FOLD_ROW_MARKER.chars().count()
        );
        // PETSCII gets no marker at all: 40 columns has no room for one, so the
        // colour carries it there and `K` prints the explanation.
        assert!(
            ERASE_FOLD_ROW_MARKER.chars().count() + 26 > 36,
            "if the marker ever fits a C64 row, put it there too"
        );
    }

    /// The terminal's copy has to fit the narrowest surface that shows it.
    #[test]
    fn test_the_warning_fits_a_40_column_c64() {
        // The width the serial screens truncate a value to on PETSCII, less the
        // indent `show_error_lines` renders with.
        const W: usize = 38;
        for line in erase_fold_warning_lines() {
            assert!(
                line.chars().count() <= W,
                "{line:?} is {} columns; a C64 does not wrap, it loses the end",
                line.chars().count()
            );
            assert!(line.is_ascii(), "{line:?} must be ASCII for a PETSCII terminal");
        }
        // Both copies must actually warn about the same thing, or one surface
        // explains the colour and another does not.
        let joined = erase_fold_warning_lines().join(" ");
        for needle in ["MODEM", "corrupt", "pass-through"] {
            assert!(joined.contains(needle), "the terminal copy never says {needle:?}");
            assert!(
                ERASE_FOLD_MODEM_WARNING.contains(needle),
                "the tooltip copy never says {needle:?}"
            );
        }
    }
