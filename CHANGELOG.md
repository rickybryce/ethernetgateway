...
- **CP/M shell — a CP/M-flavored file manager over telnet/SSH.** A new
  ...
  (validated + canonicalized; `..`/absolute/symlink escapes are refused) and
  bounded by the 8 MB transfer cap. Works identically over telnet and SSH.
  Documented in user manual §8.10; see `kernelplan.md`. (Flavor B — a real Z80
  CP/M 2.2 emulator — remains deferred.)
