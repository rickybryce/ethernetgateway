...
notices and license texts are reproduced in [`THIRD-PARTY-NOTICES.md`](THIRD-PARTY-NOTICES.md),
...
All dependencies are permissive- or GPL-compatible-licensed. This is enforced in
CI by [`cargo-deny`](https://github.com/EmbarkStudios/cargo-deny) against the
allowlist in `deny.toml` (run locally with `cargo deny check licenses`), which
rejects any GPL-3-incompatible license (e.g. `GPL-2.0-only`, AGPL) before it can
enter the tree.
